...
def test_places_without_street_view_weigh_little():
    weights = cell_weights(GRID, coverage_strength=1.0)
    assert weights[cell(40, 116)] < 0.1 * weights[cell(36, 140)]  # Beijing vs Tokyo
    gentler = cell_weights(GRID)  # the bot leans on coverage less than that by default
    assert weights[cell(40, 116)] < gentler[cell(40, 116)] < gentler[cell(36, 140)]
